...
The RedSec learning site: a free, open learning platform for offensive security.
No accounts, no trackers. Built with Astro + MDX.
...
Learning content is organized into **sections**, each a stack of MDX lessons,
rendered with a persistent left sidebar, an "on this page" TOC, and prev/next
navigation.

- Section metadata + ordering: `src/lib/learn.ts` (`SECTIONS`).
- Content: `src/content/learn/<section>/<page>.mdx`.
- Routes: `/` (landing), `/<section>` (section index), `/<section>/<page>` (lesson).
- Bulk generated lessons: `npm run generate:learn`.

### Add a page

1. Drop an MDX file in `src/content/learn/<section>/` with frontmatter:
   ```yaml
   ---
   title: My page
   order: 2
...
2. It appears in that section's sidebar and index automatically (ordered by
   `order`), with prev/next wired across the whole learning library.

### Add a section

Add an entry to `SECTIONS` in `src/lib/learn.ts`, then create
`src/content/learn/<new-section>/` with pages.
